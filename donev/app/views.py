...
class donate_now(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            description=request.POST['description']
            try:
                Donation.objects.create(donor=donor,donationname=donationname,donationpic=donationpic,collectionloc=collectionloc,
...
def index_volunteer(request):
    ...
    totalNotRecDonation=Donation.objects.filter(volunteer=volunteer,status="Donation delivered successfully").count()
...
class donationrec_detail(View):
    ...
    def post(self,request,pid):
        ...
        try:
            ...
            print("Error")
        return render(request,'donationrec-detail.html',localals())
